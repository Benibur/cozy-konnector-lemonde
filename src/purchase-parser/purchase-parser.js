...
module.exports = $ => {
  ...
  for (let i = 1; i < purchasesTable[0].length; i++) {
    let purchaseDate = purchasesTable[0][i].match(dateReg)[0]
    purchasesTable[0][i] = purchaseDate
  }

  /* sanitize the description column */
  for (let i = 1; i < purchasesTable[1].length; i++) {
    purchasesTable[1][i] = Cheerio.load(purchasesTable[1][i])
      .text()
      .replace(/\n/, '')
      .trim()
  }
  ...
  for (let i = 1; i < purchasesTable[2].length; i++) {
    purchasesTable[2][i] = Cheerio.load(purchasesTable[2][i])
      .text()
      .replace(/\n/, '')
      .trim()
  }
  ...
  for (let i = 1; i < purchasesTable[3].length; i++) {
    let amount = Cheerio.load(purchasesTable[3][i])
      .text()
      .match(/\d+[.,]?\d*/g)[0]
  ...
  for (let i = 1; i < purchasesTable[4].length; i++) {
    const invoicesUrl = Cheerio.load(purchasesTable[4][i])('.itemfleche').attr(
      'href'
    )
    if (!invoicesUrl) {
      purchasesTable[4][i] = ''
