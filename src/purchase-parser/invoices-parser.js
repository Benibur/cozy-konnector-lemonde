...
module.exports = (invoicesBody$)=>{
  ...
  for (var i = 1; i < invoicesTable[0].length; i++) {
  ...
  for (var i = 1; i < invoicesTable[1].length; i++) {
    let invoiceCell = invoicesTable[1][i]
    invoiceCell = Cheerio( '<div>' + invoiceCell + '</div>')
    invoicesTable[1][i] = invoiceCell.text().replace(/\\n/,'').trim()
  }

  // sanitize date
  for (var i = 1; i < invoicesTable[2].length; i++) {
    let invoiceCell = invoicesTable[2][i]
    invoiceCell = Cheerio( '<div>' + invoiceCell + '</div>')
    invoicesTable[2][i] = invoiceCell.text().replace(/\\n/,'').trim()
  }

  // sanitize column with invoice pdf link
  for (var i = 1; i < invoicesTable[3].length; i++) {
