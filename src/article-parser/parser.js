const
  ...
  getDescriptionTemplate = require(path.join(__dirname, './templates/description.template.js')),
  getTitleTemplate       = require(path.join(__dirname, './templates/title.template.js'      )),
...
module.exports = (article)=>{
  ...
  try {
    ...
    art$.children().each((i,el)=>{
      ...
      /* Article description */
      }else if (sign.contains('P.description') || sign.contains('DIV.description') || sign.contains('P.description-article') ) {
        headContentData.description.push( getDecoredText(el$))
        return

      /* Article signature */
      }else if (sign.contains(['P','.bloc_signature']) || sign.contains('DIV.credits') || sign.contains('P.signature') || sign.contains('P.content-byline') || sign.contains('P.content-metaline') ) {
        ...
        el$.children().each((i,el)=>{
          ...
          /* Article body - ASIDE.fenetre TODO */
            ...
            el$.children().each( (i,el) => {
              ...
              if (sign.isEqual(['SPAN', '.titre']) || sign.contains(['H3']) ) {
                bodyContent.push({
                  style: 'fenetre',
            			text: getDecoredText(el$, true,getStyle('fenetre', {bold:true}))
            		})
                return
          ...
          /* Plusieurs cas possibles */
            ...
            if (new Signature(el$.children().first()).contains('P.lire')){
              bodyContent.push({
                style: 'SeeAlso',
                text: getDecoredText(el$.children().first(), true,getStyle('SeeAlso'))
              })
              return
            }
            ...
            if (iframe$.length > 0 && iframe$.attr('src').includes('scribd.com')) {
              const textNode = el$.find('A').first()
              bodyContent.push({
                text:['Consultez les documents sur Scribd :\n',
                  {
                    text : textNode.text(),
          ...
          /* Article body - tweet */
            ...
            bodyContent.push({
              margin: [10,0,40,0],
                ...
                widths:[22,'*'],
                body: [
                  [{image:'tweetIcone',width:20, margin:[0,20,5,0]},{text:txt, }]
                ]
              },
              layout: 'noBorders'
        		})
          ...
          }else if (sign.isEqual('BLOCKQUOTE.citation') || sign.isEqual('BLOCKQUOTE' )) {
            bodyContent.push({
              margin: [20,20,20,10],
              style:{italics: true,
                color:'#777777'
          ...
          /* Article body - ordered list */
            ...
            el$.children().each((i,el)=>{
              bodyContent.push({text: [(i+1) + " - "].concat(getDecoredText(Cheerio(el))), margin:[0,10,0,0]})
            })
          ...
          /* Article body - unordered list */
            ...
            // TODO for now, pdfmake allows only string in LI items (no styles) :   https://github.com/bpampuch/pdfmake/issues/881
            el$.children().each((i,el)=>{
              bodyContent.push({text: ["- "].concat(getDecoredText(Cheerio(el))), margin:[0,10,0,0]})
            })

          }else{
            throw new ArticleParsingError(`Html format of the article body is nos as expected\nUnknown element is : ${logElement(el$)}`)
          }
        })
        return

      }else{
        throw new ArticleParsingError(`html format of the article is nos as expected\nUnknown element is :${logElement(el$)}`)
      }
    })
    ...
    if (seeAlsoBloc$.length > 0) {
      ...
      seeAlsoBloc$.find('LI').each((i,el)=>{
        body.push( [{border:[false,false,false,false],
    ...
    if (article.url) {
      headContentData.signature.push({text:'\nArticle en ligne', link:article.url, style:'link'})
    }
    ...
    return (
      Promise.map(imagesToFetch, image => {
        return Request({ uri: image.url, encoding: 'binary', resolveWithFullResponse: true})
        .then( res => {
          let
  ...
  // in case of an error during parsing, catch the error as a rejected promise
    ...
    return Promise.reject('\n'+e.toString() + `article url : ${article.url}` + '\nfull article structure:\n' + logElement(art$) + '\stacktrace:\n'+e.stack)
  }
}
