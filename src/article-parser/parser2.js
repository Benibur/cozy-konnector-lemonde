...
module.exports = article => {
  ...
  const SELECTORS_TO_REMOVE = [
    ...
    '#header-page',
    '#nav.conteneur-nav',
    '.super_global > .colonnette',
    '#footer-page.univers-sombre'
  ]
  ...
  videos$.each((i, el) => {
    ...
    if (provider === 'youtube') {
      ...
      el$.append(
        `<iframe data-provider="youtube" data-title="${el$.prop(
          'data-title'
        )}" allowfullscreen="1" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" title="YouTube video player" class="js_player" src="https://www.youtube.com/embed/${videoId}?autoplay=0&enablejsapi=1&origin=https%3A%2F%2Fwww.lemonde.fr&widgetid=1"></iframe>`
      )
    } else if (provider === 'digiteka') {
      el$.append(
        `<iframe src="https://www.ultimedia.com/deliver/generic/iframe/src/${videoId}/"></iframe>`
      )
    }
  })
