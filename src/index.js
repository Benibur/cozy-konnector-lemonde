...
if (DEBUG_MODE) {
      ...
      TESTS_DIRECTORIES.forEach(d=>{        // create test directories if it
...
function authenticate(username, password) {
  ...
  // 2. Create a session
    .then($ => {
      ...
      .catch((err)=>{
        log('error', 'Server connection failed' + err)
...
function retriveInvoices() {
  ...
  .then($ => {
    if (DEBUG_MODE) { FS.writeFileSync(PURCHASES_FILES_PATH + 'purchases-page.html', $.html()) }
    ...
    .mapSeries(invoiceRow => {
      ...
      if (DEBUG_MODE) { // then we save the invoice for tests
        ...
        .then(resp=>{
          ...
          log('debug', `Save invoice ${invoiceRow[3]} in file : ${filepath}`)
          FS.writeFileSync(filepath, resp.body)
          FS.writeFileSync(filepath + '.url', invoiceRow[3])
        })
        .delay(1000) // if too fast, the server responds a 503...
      }
      ...
      .then(()=>{
        return new Promise(function(resolve) {setTimeout(() => resolve("done"), 1000)})
      }) // if too fast, the server responds a 503...
  ...
  if (false && DEBUG_MODE) { // for debug. To test a specific article (just change its url)
    ...
    .then( body => {
      ...
      if (DEBUG_MODE) {
        ...
        FS.writeFile(ARTICLES_LIST_PATH,`number of articles = ${articlesList.length}\n` + JSON.stringify(articlesList), ()=>{})
      }
      ...
      if (article.url !== null) {
        if (DEBUG_MODE) {
          ...
          */
          ...
          log('DEBUG', `Save article in : $(filename).html, $(article.url)`)
          const fs = require('fs')
